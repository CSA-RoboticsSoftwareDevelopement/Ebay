...
import { useAuth } from "../../../context/AuthContext"; // Corrected import path based on typical Next.js structure

const BACKEND_SERVER_URL = process.env.NEXT_PUBLIC_BACKEND_SERVER_URL;

// Define the Product interface outside the component for better readability and reusability
interface Product {
  id: string;
  title: string;
  description: string;
  ...
  costPrice: number;
  shipping: number;
  ebayFees: number; // This might be 0 for Amazon products
  profit: number;
  profitMargin: number;
  roi: number;
  imageUrl: string;
  ...
  source: "eBay" | "Amazon" | "Dummy"; // Add a 'source' field to identify the origin
  competitorData: {
    id: string;
    avgPrice: number;
    avgShipping: number;
  ...
  condition: string;
  category: string; // This might be 'Unknown' for now if not provided by the API
}

export default function Products() {
  const { user } = useAuth();
  const EBAY_INVENTORY_API = `${BACKEND_SERVER_URL}/api/ebay/products/inventory?user_id=${user?.id}`;
  const AMAZON_TEST_API = `${BACKEND_SERVER_URL}/api/amazon/test`; // Your Amazon API endpoint
  ...
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("");
  const [sourceFilter, setSourceFilter] = useState<"All" | "eBay" | "Amazon">(
    "All"
  ); // New state for source filter
  const [isAddProductModalOpen, setIsAddProductModalOpen] = useState(false);
  const [refreshTrigger, setRefreshTrigger] = useState(0);

  // Your dummy products (unchanged, just ensuring 'source' is set)
  const dummyProducts: Product[] = [
    {
      id: "dummy-1",
      title: "Wireless Headphones",
  ...
  useEffect(() => {
    ...
    // Interface for eBay inventory items (from your previous code)
    interface EbayInventoryItem {
      sku: string;
      product: {
        title: string;
        description: string;
        mpn: string; // Assuming mpn might be used for price in your current setup
        imageUrls: string[];
      };
      availability: {
        shipToLocationAvailability: {
    ...
    // Corrected Interface for Amazon inventory summaries based on your screenshot
    interface AmazonInventorySummary {
      ...
      lastUpdateTime: string; // ISO string date
      inventoryDetails: {
        totalReservableQuantity: number;
      };
    }

    // Interface for the overall Amazon API response structure
    ...
    async function fetchAllProducts() {
      ...
      // Fetch eBay Products
      try {
        const ebayResponse = await fetch(EBAY_INVENTORY_API);
        if (!ebayResponse.ok) {
          throw new Error(
            `Failed to fetch eBay inventory: ${ebayResponse.statusText}`
          );
        }
        const ebayData = await ebayResponse.json();
        const ebayInventoryItems = ebayData.inventory?.inventoryItems || [];
        console.log("✅ eBay Inventory API response:", ebayData);

        const formattedEbayProducts: Product[] = ebayInventoryItems.map(
          (item: EbayInventoryItem) => ({
            id: item.sku,
            title: item.product?.title || "Untitled eBay Product",
            description: item.product?.description || "No description available",
            price: !isNaN(parseFloat(item.product?.mpn || "0"))
              ? parseFloat(item.product?.mpn || "0")
              : 0,
            ...
            imageUrl:
              item.product?.imageUrls?.[0] ||
              `https://placehold.co/400x300?text=${encodeURIComponent(
                item.product?.title || "Untitled eBay Product"
              )}`,
            listingStatus: "Active",
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
        ...
        allFetchedProducts = [...allFetchedProducts, ...formattedEbayProducts];
      } catch (err: unknown) {
        if (err instanceof Error) {
          fetchErrors.push(`eBay Fetch Error: ${err.message}`);
        } else {
          fetchErrors.push("An unexpected error occurred during eBay fetch.");
        }
      }

      // Fetch Amazon Products
      try {
        const amazonResponse = await fetch(AMAZON_TEST_API);
        if (!amazonResponse.ok) {
          throw new Error(
            `Failed to fetch Amazon data: ${amazonResponse.statusText}`
          );
        }
        const amazonData: AmazonApiResponse = await amazonResponse.json();
        const amazonInventorySummaries =
          amazonData.payload?.inventorySummaries || [];
        console.log("✅ Amazon API response:", amazonData);

        const formattedAmazonProducts: Product[] =
          amazonInventorySummaries.map((item: AmazonInventorySummary) => ({
            id: item.asin,
            title: item.productName || "Untitled Amazon Product",
            description: "No description available from Amazon summary",
            price: 0,
            currency: "USD",
            ...
            profitMargin: 0,
            roi: 0,
            imageUrl: `https://placehold.co/400x300?text=${encodeURIComponent(
              item.productName || "Amazon Product"
            )}`,
            listingStatus: "Active",
            createdAt: item.lastUpdateTime || new Date().toISOString(),
            updatedAt: item.lastUpdateTime || new Date().toISOString(),
            source: "Amazon",
            competitorData: {
              id: item.asin,
              avgPrice: 0,
              ...
              avgImageCount: 0,
              competitorCount: 0,
              lastUpdated: new Date(item.lastUpdateTime),
            },
        ...
        allFetchedProducts = [
          ...allFetchedProducts,
          ...formattedAmazonProducts,
        ];
      } catch (err: unknown) {
        if (err instanceof Error) {
          fetchErrors.push(`Amazon Fetch Error: ${err.message}`);
        } else {
          fetchErrors.push("An unexpected error occurred during Amazon fetch.");
        }
      }

      // Determine products to set:
      // If any real products were fetched, use them.
      // Otherwise, if there were fetch errors and no real products, use dummy products.
      let productsToDisplay: Product[] = [];
      if (allFetchedProducts.length > 0) {
        // Use a Map to ensure unique products, prioritizing real data if IDs conflict
        const uniqueProductsMap = new Map<string, Product>();
        allFetchedProducts.forEach((product) =>
          uniqueProductsMap.set(product.id, product)
        );
        productsToDisplay = Array.from(uniqueProductsMap.values());
      } else if (fetchErrors.length > 0) {
        // If there were errors and no real products, display dummy data
        productsToDisplay = dummyProducts;
        setError(
          `Some data could not be fetched: ${fetchErrors.join(
            "; "
          )}. Showing demo data.`
        );
      } else {
        // No real products and no specific errors (e.g., API returned empty array)
        productsToDisplay = dummyProducts;
        setError("No inventory found from any source. Showing demo data.");
      }
    ...
    fetchAllProducts();
  }, [refreshTrigger, user]); // Dependencies
  ...
  const handleAddProduct = (newProduct: Product) => {
    setProductsData((prevProducts) => {
      ...
      const updatedProduct: Product = {
        ...
        createdAt: newProduct.createdAt || new Date().toISOString(),
        updatedAt: newProduct.updatedAt || new Date().toISOString(),
        source: newProduct.source || "Dummy", // Ensure source is set for new products
        competitorData: {
          id: newProduct.id,
          avgPrice: newProduct.price,
          avgShipping: newProduct.shipping,
          lowestPrice: newProduct.price,
          highestPrice: newProduct.price,
          avgSellerFeedback: 0,
          avgListingPosition: 0,
          avgImageCount: newProduct.imageUrl ? 1 : 0,
          competitorCount: 0,
          lastUpdated: new Date(),
        },
  ...
  // Filter products based on search query, status filter, and NEW source filter
  const filteredProducts = productsData.filter((product) => {
    const matchesSearch = (product.title || "")
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
  ...
  const handleUpdateProduct = (
    ...
    console.log("Updating product:", productId, updates);
    alert(`Product ${productId} updated (simulated)`);
    // In a real application, you'd make an API call to update the product in your backend
    // and then re-fetch or update the state accordingly.
    setRefreshTrigger((prev) => prev + 1); // Trigger re-fetch to see real updates
  };
  ...
  // Render logic for guest users (unchanged, still shows dummy products)
  if (!user?.id) {
    return (
      <div>
        <div className="text-center text-white py-6">
      ...
      <div className="bg-neutral-800 rounded-lg shadow p-4 mb-12 mt-5">
        <div className="flex flex-col md:flex-row gap-4 mb-4">
          ...
          <div className="flex-1 flex gap-2">
            <button
              className={`btn ${
                sourceFilter === "All" ? "btn-primary" : "btn-secondary"
              }`}
            ...
            <button
              className={`btn ${
                sourceFilter === "eBay" ? "btn-primary" : "btn-secondary"
              }`}
            ...
            <button
              className={`btn ${
                sourceFilter === "Amazon" ? "btn-primary" : "btn-secondary"
              }`}
